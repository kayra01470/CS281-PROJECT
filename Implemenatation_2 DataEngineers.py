...
def control_these_comments(values):
    ...
    global show_name  # Add this line
    show = values['show']
    if show == ' ':
        sg.popup('Please choose a film first')
    else:
        show_name = show[1]  # Extract the selected show name
        comment = []
...
def deleting_comment(values):
    ...
    if comment:
        ...
        # Re-query the database to get the updated list of comments
        ...
        # Update the comment list in the window
