...
def window_show_comment():
    ...
    for row in cur.execute('SELECT ID,name from Show'):
        shows.append(row)
    layout = [[sg.Text('Show:'), sg.Combo(shows, key='show'), sg.Button('See Comments')],
              [sg.Button('Return To Main')]]
...
def control_these_comments(values):
     ...
     if show == ' ':
        ...
        for row in cur.execute('''SELECT c.comment
            ...
            layout = [[sg.Listbox(comments, size=(40,10), key='comment')],
...
def deleting_comment(values):
    ...
    comment2 = values['comment']
    window.Element('comment').Update(values=comment2)
